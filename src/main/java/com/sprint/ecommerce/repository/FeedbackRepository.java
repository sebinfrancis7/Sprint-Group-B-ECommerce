...
//	select count(*) from feedback f join orders o on f.order_id = o.order_id where o.product = 1;
	...
	@Query("select new com.sprint.ecommerce.helpers.FeedbackResponse(f.feedbackId,f.rating, f.feedback, f.dateCreated) from Feedback f where f.order.customer.custId = :id order by f.order.orderId desc")
	public List<FeedbackResponse> getFeedbackFromCustomerID(@Param("id") int id);

	@Query("select new com.sprint.ecommerce.helpers.FeedbackResponse(f.feedbackId,f.rating, f.feedback, f.dateCreated) from Feedback f where f.order.product.prodId = :id order by f.rating desc")
